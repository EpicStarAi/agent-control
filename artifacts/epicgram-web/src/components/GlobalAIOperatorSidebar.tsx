...
export function GlobalAIOperatorSidebar() {
  ...
  // ---- Operator command intent recognition -> UI navigation (open chats / private filter / etc.) ----
  // Advisory-only routing: this never sends anything itself. UI-actionable intents are forwarded to
  // the Telegram workspace via a CustomEvent and the workspace reports back what it did.
  const reqSeqRef = useRef(0);
  const runOperatorCommand = (q: string) => {
    const intent = parseOperatorIntent(q);

    if (intent.kind === "help") { pushChat("ai", t("sb.help", loc)); return true; }
    if (intent.kind === "show_send_status") {
      const line = tgReady === true
        ? "Telegram Runtime готов. Режим отправки: MANUAL_ONLY, автоотправка и массовая отправка запрещены, каждое сообщение требует ручного подтверждения оператора."
        : tgReady === false
          ? "Telegram Runtime не готов, live-отправка недоступна. Режим отправки в любом случае: MANUAL_ONLY, только по approval."
          : "Статус Telegram Runtime ещё неизвестен. Режим отправки: MANUAL_ONLY, только по approval.";
      pushChat("ai", line);
      return true;
    }
    if (intent.kind === "clarify_chat_name") { pushChat("ai", t("sb.clarifyChatName", loc)); return true; }
    if (UI_ACTION_INTENTS.indexOf(intent.kind) >= 0) {
      dispatchUiAction(intent as any, intent.kind);
      return true;
    }
    return false;
  };

  const dispatchUiAction = (intent: { kind: string; query?: string }, label: string) => {
    reqSeqRef.current += 1;
    const reqId = "opcmd_" + Date.now() + "_" + reqSeqRef.current;
    pushChat("ai", t("sb.running", loc) + ": " + label + "…");
    let settled = false;
    const onResult = (e: any) => {
      const d = e && e.detail;
      if (!d || d.reqId !== reqId || settled) return;
      settled = true;
      window.removeEventListener("deepinside:operator-result", onResult);
      pushChat("ai", d.message || (d.success ? "Готово." : t("sb.notFoundChat", loc)));
    };
    window.addEventListener("deepinside:operator-result", onResult);
    setTimeout(() => {
      if (settled) return;
      settled = true;
      window.removeEventListener("deepinside:operator-result", onResult);
      pushChat("ai", "Не удалось выполнить команду в текущем разделе (возможно, чаты ещё загружаются). Открой Telegram-клиент вручную и повтори.");
    }, 8000);
    try {
      window.dispatchEvent(new CustomEvent("deepinside:operator-command", { detail: { reqId, intent: intent.kind, query: intent.query } }));
    } catch {}
  };
    ...
    <div className="min-h-0 flex-1 space-y-2 overflow-auto p-2">
      ...
      {card(t("sb.chat", loc), <div className="space-y-1">
        ...
        <div className="flex gap-1">
          <input value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") send(); }} placeholder={t("sb.commandsHint", loc)} className="min-w-0 flex-1 rounded bg-black/40 px-2 py-1 text-[10px] text-tg-text" />
