...
export async function GET() {
  const auth = await requirePrincipal("/api/v1/telegram/saved", "GET");
  if (!auth.ok) return auth.response;

  const accountId = await resolveBoundAccountId(auth.principal);
  if (!accountId) return guardedJson({ ...safeEmptyState("no_binding"), saved: [] }, 200);

  return guardedJson({ ...safeEmptyState("no_binding"), saved: [] }, 200);
}
