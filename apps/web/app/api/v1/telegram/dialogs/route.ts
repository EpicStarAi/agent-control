...
export async function GET() {
  const auth = await requirePrincipal("/api/v1/telegram/dialogs", "GET");
  if (!auth.ok) return auth.response;

  const accountId = await resolveBoundAccountId(auth.principal);
  if (!accountId) return guardedJson({ ...safeEmptyState("no_binding"), dialogs: [] }, 200);

  return guardedJson({ ...safeEmptyState("no_binding"), dialogs: [] }, 200);
}
