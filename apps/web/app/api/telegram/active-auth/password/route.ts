...
function rawAuthState(value: unknown): string {
  ...
  if (value && typeof value === "object" && typeof (value as { _?: unknown })._ === "string") {
    return String((value as { _: string })._);
  }
  return "";
}
...
export async function POST(req: NextRequest) {
  const auth = await requireLegacyOwnerSurface("/api/telegram/active-auth/password", "POST", "active_auth_password");
  ...
  if (activeState !== "authorizationStateWaitPassword") {
    return NextResponse.json({ ok: false, reason: "active_slot_not_waiting_password", authorizationState: activeState }, { status: 409, headers: H });
  }

  const result = await backend("/telegram/auth/2fa", { accountId: activeAccountId, password });
  password = "";
  if (!result.response.ok) {
    return NextResponse.json({ ok: false, reason: str(result.data.message) ?? "Облачный пароль не принят Telegram." }, { status: 409, headers: H });
  }
