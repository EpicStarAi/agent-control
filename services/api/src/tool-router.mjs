...
async function toolSummarizeChat(n) {
  ...
  const r = await generateDraftReply({
    ...
    history: n.history,
    instruction:
      "По предоставленной истории чата дай структуру: 1) подтверждённые факты; 2) краткое резюме; 3) открытые вопросы; 4) предложенный следующий ответ оператора. Не выдумывай результаты и не называй тест успешным, если в истории есть только команда без явного результата. Отделяй факты от предположений и опирайся только на данный контекст."
  });
